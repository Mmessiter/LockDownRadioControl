...
void InputsViewEnd()
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        ...
        if (i < 4)
            InputTrim[i] = CheckRange((GetValue(InputTrim_labels[i]) - 1), 0, 15);// trim values
        GetText(fsch_labels[i], ChannelNames[InPutStick[i]]); // get channel names
        SendValue(Progress, (((i + 1) * 100) / 16) - 1); // progress bar
    }
    CheckOutPutChannels();
    SendValue(Progress, 95);
    SaveOneModel(ModelNumber);
