...
char * AddSizeToFilename(int ff, char * size){
    ...
    while (strlen(TheFilesList[ff]) < 12) strcat(TheFilesList[ff], " ");
...
void LoadFileSelector()
{
    ...
    for (int f = 0; f < ExportedFileCounter; ++f) {
        strcpy(SizeBuf, "");
        if (strcmp(MOD, ".MOD"))  AddSizeToFilename(f, SizeBuf);
        if (!f) strcpy(buf, TheFilesList[f]); else strcat(buf, TheFilesList[f]); // first one? if not, concatenate
        if (strcmp(MOD, ".MOD")) strcat(buf, SizeBuf);
        strcat(buf, crlf);
    }
    SendOtherText(Mfilesp, buf);
    FileNumberInView = GetValue(Mfiles);
    LastFileInView   = 120;
   if (strcmp(MOD, ".MOD")) ShowFreeSpaceEtc(); // show free space etc if log files are being viewed
}
