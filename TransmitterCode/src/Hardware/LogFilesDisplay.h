...
void LogReleasedNEW(){ 
        ...
        if (Current_Y >  Previous_Current_Y) Direction = GOING_DOWN; 
        if (Current_Y <  Previous_Current_Y) Direction = GOING_UP;
        if (Current_Y == Previous_Current_Y) return;

        if (Direction == GOING_DOWN) {
            if (Current_Y > (Max_Y * SCROLLTRIGGER)){
                StartReadLine += BUFFEREDLINES;
                if (StartReadLine >= FinalReadStartLine) return;
...
{
    if (ScrollWithoutDisplaying) return; // if we are just scrolling to bottom, don't display anything
    ...
    if (ReadingaFile) SendText(t0, LogFileName);
    ReadingaFile = false;
