...
FASTRUN void ProcessRecentCommsGap() // When we know there is time, process a recent gap.
{
    ...
    { // RXV2 announced a flight-save flash write (ack item 38): the gap is
      ...
      // honestly noted in the log.
        ...
        {
            char Excused[48];
            snprintf(Excused, sizeof(Excused), "%lu ms gap excused (RX saving flight)", (unsigned long)ThisGap);
...
// ************************************************************************************************************/
void ShowGhz(char X1_pos[9], char ghz[10], uint16_t *p)
{
    char CB[150]; // COMMAND BUFFER
    char xstr[] = "xstr ";
    int x1 = xx1;
    int xd1 = 20; // this puts the decimal point right under the tick mark
    char NB[9], NB1[9], NB2[9], NB3[9], NB4[9], NB5[9], NB6[9], NB7[9], NB8[9];
    int y2 = yy1 + 255;
    int y3 = y2 + YY1EXTRA;
    char NewWhite1[15];
...
void DrawFhssBox()
{
    ...
    strcpy(X1_pos, Str(NB, x1 + p - xd1, 1));
    DrawBox(x1 - 5, y1, x2 + 5, y2, ForeGroundColour);         // Draw the outer box, double line
    DrawBox(x1 - 6, y1 - 1, x2 + 6, y2 + 1, ForeGroundColour); // Draw the outer box, double line
    ShowGhz(X1_pos, STR1GHZ, &p);
    ShowGhz(X1_pos, STR32GHZ, &p);
...
void ReadPIDs_Advanced_FromAckPayload(uint8_t n, uint8_t m)
{
    ...
    for (uint8_t i = n; i < m; ++i)
    {
        ...
        {
            PID_Advanced_Values[i] = AckPayload.Ack_Payload_byte[p + 1];
            ++p;
        }
    }
